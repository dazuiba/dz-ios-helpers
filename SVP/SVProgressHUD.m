...
+ (void)showWithStatusAndButton:(NSString*)status buttonTitle:(NSString *)btnTitle target:(id)target action:(SEL)action{
    [[SVProgressHUD sharedView] showWithStatusAndButton:status buttonTitle:btnTitle target:target action:action];
}
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event{
    CGPoint point2 = [self convertPoint:point toView:self.hudView];
    BOOL contains = CGRectContainsPoint(actionButton.frame, point2);
//    NSLog(@"hitTest: %@, %@, %@ %d",NSStringFromCGPoint(point),NSStringFromCGPoint(point2),
//          NSStringFromCGRect(actionButton.frame), contains);
    if (contains) {
        return actionButton;
    }else {
        return nil;
...
- (void)showWithStatusAndButton:(NSString*)status buttonTitle:(NSString *)btnTitle target:(id)target action:(SEL)action{
    ...
    self.stringLabel.center = NIPointMove(stringLabel.center, 0, -47);    
    
    if (self.actionButton!=NULL && !self.actionButton.hidden) {
        self.actionButton.frame = CGRectMake(0, 0, 90, 25);
        self.actionButton.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, CGRectGetMaxY(self.stringLabel.frame)+34);
    }
...
- (void)dismissWithStatus:(NSString*)string error:(BOOL)error {
	[self dismissWithStatus:string error:error afterDelay:kHUDDuration];
}
...
//        actionButton.layer.opacity = .8;
        actionButton.hidden = YES;
        [self.hudView addSubview:actionButton];
